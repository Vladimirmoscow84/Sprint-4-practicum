...
func RunningSpentCalories(action int, weight, duration float64) float64 {
    // ваш код здесь
    return ((runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration)) * weight/mInKm * duration * minInH)
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
    // ваш код здесь
    return ((walkingCaloriesWeightMultiplier * weight + (math.Pow((meanSpeed(action, duration)) * kmhInMse, 2)/ height) * walkingSpeedHeightMultiplier * weight) * duration * minInH)
}
